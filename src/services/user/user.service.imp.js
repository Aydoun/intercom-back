...
export const searchUser = (term) => {
  return UserModel.paginate(
    { $text: { $search: term }, status: "Active" },
...
export const updateUserImp = (id, newData) => {
  return UserModel.findByIdAndUpdate(id, newData).then((document) => {
    const newDocument = Object.assign({}, document.toObject(), newData);
    const { bio, name } = newDocument;

    if (!newDocument.profileCompleted && bio && name) {
...
export const deleteUserImp = (id) => {
  return UserModel.updateOne({ _id: id }, { status: "Inactive" }).then(
    () => ({})
  );
};

export const hardDeleteUserImp = async (id) => {
  try {
...
export const registerUserImp = (name, email, password) =>
...
export const loginUserImp = (email, password) =>
  UserModel.findOne({ email })
    ...
    .then((user) => {
      ...
      return comparePasswords(password, user.password).then((isMatch) => {
        if (!isMatch) {
...
export const changePasswordImp = (id, oldPassword, newPassword) =>
...
export const getIntersection = (mainUser, otherUsers) => {
  ...
  return UserModel.find({ _id: { $in: [mainUser, ...toArray] } }).then(
    (users) => {
      if (users.length > 0) {
        ...
        users.slice(1).forEach((user) => {
          response[user._id] = _intersectionWith(
            user.plans,
            mainUserPlans,
